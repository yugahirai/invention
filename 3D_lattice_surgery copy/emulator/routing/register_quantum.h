...
public:
    QuantumRegister(uint32_t id, string _name, Position3D _pos) : m_id(id), name(_name), status(QuantumRegisterStatus::FREE), reserved_time(0){
        pos_list.push_back(_pos);
    }

    virtual Position3D get_position() const{
...
public:
    MagicFactory(uint32_t id, string _name, Position3D _pos, uint32_t _code_beat_for_prepare) 
...
private:
    ...
    QubitSpace qubit_space;
    const uint32_t factory_time;

public:
    QuantumRegisterMap(uint32_t width, uint32_t height, uint32_t depth, uint32_t _factory_time) : qubit_space(width, height, depth), factory_time(_factory_time){};

    virtual void add_register(string name, Position3D position){
        if (qreg_list.find(name) != qreg_list.end()){
            string message = "register " + name + " is already allocated.";
            throw invalid_argument(message);
        }
        if (!qubit_space.is_available(position)){
    ...
    virtual void add_magic_factory(string name, Position3D position){
        if (qreg_list.find(name) != qreg_list.end()){
            string message = "register " + name + " is already allocated.";
            throw invalid_argument(message);
        }
    ...
    virtual const QubitSpace& get_qubit_space() const{
        return qubit_space;
    }

    virtual const map<string, Position3D> get_available_position_list() const{
        map<string, Position3D> result;
        for (const auto& qreg : qreg_list){
            if (qreg.second.is_available()){
                string name = qreg.first;
                Position3D pos = qreg.second.get_position();
    ...
    virtual const map<string, Position3D> get_available_factory_list() const{
        map<string, Position3D> result;
        for (const auto& factory : factory_list){
            if (factory.second.is_available()){
                string name = factory.first;
                Position3D pos = factory.second.get_position();
                result.insert(make_pair(name, pos));
            }
        }
        return result;
    ...
    virtual void resolve(const InstructionResolveInfo& info){
        for (auto val : info.new_qubit) {
            ...
            qubit_space.allocate_parent_block(val.second);
        }
        for (auto val : info.new_magic_factory){
            add_magic_factory(val.first, val.second);
            qubit_space.allocate_magic_block(val.second);
        }
        for (auto pos : info.lock_position) {
            qubit_space.allocate_children_block(pos, info.lock_lifetime);
        }
        ...
        for (auto name : info.remove_qubit) {
            ...
            qubit_space.deallocate_parent_block(pos);
    ...
    virtual string get_floor_string() const{
        return qubit_space.get_floor_string();
    }
