...
public:
    bool success = false;
    int lock_lifetime = 0;
    vector<Position2D> lock_position;
    vector<pair<string, Position2D>> new_qubit;
    vector<pair<string, Position2D>> new_magic_factory;
...
public:
    InstructionBase(string _inst_name, InstructionType _inst_type, const vector<string>& _qreg_names, const vector<string>& _creg_cond, string _creg_name_out, bool _require_qreg_resolved)
        ...
        std::sort(creg_cond.begin(), creg_cond.end());
        std::sort(qreg_names.begin(), qreg_names.end());
    }
    virtual ~InstructionBase() {};
    virtual InstructionResolveInfo resolve_topology(const map<string, Position2D>& qubit_position, const QubitPlane& qubit_plane, const map<string, Position2D>& factory_position) const = 0;
    virtual bool resolve_creg_dependency(const vector<string>& creg_ready) const {
        return std::includes(creg_ready.begin(), creg_ready.end(), creg_cond.begin(), creg_cond.end());
    }
    virtual bool resolve_qreg_dependency(const map<string, Position2D>& qreg_info) const {
        ...
        if (require_qreg_resolved) {
            return std::includes(qreg_resolved.begin(), qreg_resolved.end(), qreg_names.begin(), qreg_names.end());
        }
        else {
            bool flag = true;
