...
private:
    ...
    virtual InstructionResolveInfo _resolve_topology_with_magic(
        ...
        const vector<pair<Position2D, PathConnectionType>>& residual_,
        const map<string, Position2D>& factory_position
    ) const {

        InstructionResolveInfo info;
        info.success = false;
        vector<pair<Position2D, PathConnectionType>> residual(residual_);

        for (const auto& factory : factory_position) {

            residual.push_back(make_pair(factory.second, PathConnectionType::BOTH));
            auto result = qubit_plane.find_path(org, org_type, residual);

            #ifdef DUMP_SURGERY_PATH
            if (result.first) {
                ofstream ofs("_temp2.txt", ios::app);
                ofs << residual[0].first.x << " " << residual[0].first.y << " ";
                for (auto val : result.second) {
                    ofs << val.x << " " << val.y << " ";
                }
                ofs << org.x << " " << org.y << endl;
                ofs.close();
            }
            #endif
    ...
    virtual InstructionResolveInfo _resolve_topology_without_magic(
        const QubitPlane& qubit_plane, Position2D org, PathConnectionType org_type, const vector<pair<Position2D, PathConnectionType>>& residual
    ) const {
        auto result = qubit_plane.find_path(org, org_type, residual);
...
public:
    static InstructionSurgery* create_instruction(
        ...
        std::sort(qreg_name_and_type.begin(), qreg_name_and_type.end());
    ...
    static InstructionSurgery* parse(const vector<string>& operands, const vector<string>& creg_cond, string creg_output, bool use_magic, uint32_t _lock_time) {
        if (operands.size() % 2 == 1) {
            throw runtime_error("invalid format surgery instruction");
        }
    ...
    virtual InstructionResolveInfo resolve_topology(
        ...
        vector<pair<Position2D, PathConnectionType>> residual;
        for (uint32_t idx = 1; idx < qreg_names.size(); ++idx) {
            Position2D trg = qubit_position.at(qreg_names[idx]);
            PathConnectionType trg_type = convert_connection(connection[idx]);
            residual.push_back(make_pair(trg, trg_type));
        }

        if (use_magic) {
            return _resolve_topology_with_magic(qubit_plane, org, org_type, residual, factory_position);
        }
        else {
            return _resolve_topology_without_magic(qubit_plane, org, org_type, residual);
        }
    }
