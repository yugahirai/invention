...
def create_control_qrom(c: Circuit, control: list[str], target: list[str], ancilla: list[str], gcnt: str, pauli_list: list[tuple[float, tuple]]) -> Circuit:
    ...
    for step in range(2**num_control):
        ...
        for ind in range(rev):
            ...
            add_end_toffoli(c, control[cur-1], fix_anc[cur-1], fix_anc[cur], 0, 0)
            cur -= 1
...
def create_control_simple_qrom(c: Circuit, control: list[str], target: list[str], ancilla: list[str], gcnt: str, pauli_list: list[tuple[float, tuple]]) -> Circuit:
    """Add QROM to registers
...
def create_control_dist_simple_qrom(c: Circuit, control: list[str], target: list[str], ancilla: list[str], gcnt: str, pauli_list: list[tuple[float, tuple]], num_dup: int) -> Circuit:
    """Add QROM to registers
    ...
    num_control = len(control)
    dups = [control] + [[f"_{d+1}_{c}" for c in control] for d in range(2**num_dup-1)]
    # print(dups)
    ...
    fix_ancs = [fix_anc_org] + [[f"_{d+1}_{c}" if c is not None else None for c in fix_anc_org] for d in range(2**num_dup-1)]
    ...
    for step in range(2**(num_control-num_dup)):
        vals = _get_bits(step, num_control-num_dup)
        # c.add_gate({"name": "barrior", "targets": []})
        for ind in range(num_dup, num_control):
            for di in range(2**num_dup):
                add_start_toffoli(c, dups[di][ind], fix_ancs[di][ind],  fix_ancs[di][ind+1], vals[ind-num_dup], 1)
        ...
        c.add_gate({"name": "barrior", "targets": []})
        for ind in range(num_control-1, num_dup-1, -1):
            for di in range(2**num_dup):
                add_end_toffoli(c, dups[di][ind], fix_ancs[di][ind],  fix_ancs[di][ind+1], vals[ind-num_dup], 1)
...
def create_control_dist_qrom(c: Circuit, control: list[str], target: list[str], ancilla: list[str], gcnt: str, pauli_list: list[tuple[float, tuple]], num_dup: int) -> Circuit:
    """Add QROM to registers
...
def create_control_dist_qrom_improve(c: Circuit, control: list[str], target: list[str], ancilla: list[str], gcnt: str, pauli_list: list[tuple[float, tuple]], num_dup: int) -> Circuit:
    """Add QROM to registers
...
def create_control_dist_qrom_improve_earlydrop(c: Circuit, control: list[str], target: list[str], ancilla: list[str], gcnt: str, pauli_list: list[tuple[float, tuple]], num_dup: int) -> Circuit:
    """Add QROM to registers
...
def create_qrot(c: Circuit, control: list[str], target: str, ancilla: list[str], angle_list: list[float, str]) -> Circuit:
    """Add QROM to registers
    ...
    Args:
        ...
        target (list): target registers
        ancilla (list): ancilla registers
        angle_list (list): _description_

    Returns:
        Circuit: _description_
    """
    assert(len(control) == len(ancilla))
    assert(2**len(control) == len(angle_list))

    if len(control) == 0:
        angle, target_pauli = angle_list[0]
        if target_pauli == "X":
            add_RX(c, target, angle)
        if target_pauli == "Y":
            add_RY(c, target, angle)
        if target_pauli == "Z":
            add_RZ(c, target, angle)
        return
    ...
    cur = num_control
    for step in range(2**num_control):
        angle, target_pauli = angle_list[step]
        if target_pauli == "X":
            add_CRX(c, accum, target, angle)
        if target_pauli == "Y":
            add_CRY(c, accum, target, angle)
        if target_pauli == "Z":
            add_CRZ(c, accum, target, angle)
...
def test_qrot():
    ...
    ancilla = ["a1", "a2", "a3"]
    target = "t1"
    angle_list = [(0.1, "X") for _ in range(8)]
    create_qrot(c, control, target, ancilla, angle_list)
    for gate in c.gates:
        print(gate)
...
def test_qrom():
    ...
    # create_control_simple_qrom(c, control, target, ancilla, None, pauli_list)
    # create_control_dist_simple_qrom(c, control, target, ancilla, None, pauli_list, 3)
    # create_control_dist_qrom(c, control, target, ancilla, None, pauli_list, 3)
    # create_control_dist_qrom_improve(c, control, target, ancilla, None, pauli_list, 1)
    create_control_dist_qrom_improve_earlydrop(c, control, target, ancilla, None, pauli_list, 0)
    import pprint
    pprint.pprint(c.gates)
    ...
    for ind in range(2**3):
        print(rvals)
        check(3, rvals[ind], ind)


def test_qrom_drop(num_control, num_dups, num_task):
    c = Circuit()
    ...
    for step in range(num_task, block_size):
        for thread_index in range(num_threads):
            item = pauli_list[thread_index*block_size + step]
            pauli_list[thread_index*block_size + step] = (0, [])
            rvals[thread_index*block_size + step] = 0
    # print(pauli_list)
    
    # create_control_dist_qrom_improve(c, control, target, ancilla, None, pauli_list, num_dups)
    create_control_dist_qrom_improve_earlydrop(c, control, target, ancilla, None, pauli_list, num_dups)
