...
const int MAX_ITERATIONS = 2000000;   // 最大反復回数
...
int main() {
    ...
    // 必要なグリッドサイズを計算
    int gridSize = static_cast<int>(6*std::sqrt(std::ceil(qubitCount/14.0)));
    ...
    for (int i = 0; i < qubitCount; ++i) {
        ...
        if (count%6 == 0){
            q.x = (qubit_num+1)*4/3 + 2*(qubit_num/3);
            q.y = count;
            if (q.x >= GRID_SIZE){
                count++;
                qubit_num = 0;
        ...
        if (count%6 == 1){
            q.x = (qubit_num)*4/3 + 2*((qubit_num + 1)/3);
            q.y = count;
            if (q.x >= GRID_SIZE){
                count++;
                qubit_num = 0;
        ...
        if (count%6 == 2){
            q.x = (qubit_num +2)*4/3 + 2 + 2*((qubit_num + 3)/3) - 6;
            q.y = count;
            if (q.x >= GRID_SIZE){
                count++;
                qubit_num = 0;
