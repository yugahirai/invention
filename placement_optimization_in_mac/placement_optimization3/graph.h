...
public:
    Node(const std::string& id, double x = 0, double y = 0, bool has_edges = false) 
        : id(id), x(std::rand() % 200 - 100), y(std::rand() %200 - 100), has_edges(has_edges){}
...
public:
    ...
    void layoutGraph() {
        ...
        while (total_movement > epsilon) { // 多くの反復で座標を調整
            ...
            for (const auto& edge : edges) {
                ...
                // 重みが大きいほどノード同士を近づける
                double force = pow(edge.get_weight(),2)*pow(distance, 1)*0.0001*0.001;
                double dp_x = force*(dx/std::abs(dx));
                double dp_y = force*(dy/std::abs(dy));
    ...
    void exportToDOT(const std::string& filename) {
        ...
        file << "    node [shape=circle, width=0.5, height=0.5, fixedsize=true, fontsize=5];\n";  // 文字を小さく設定
