...
public:
    Node(const std::string& id, double x = 0, double y = 0, bool has_edges = false) 
        : id(id), x(std::rand() % (2*g) - g), y(std::rand() % (2*g) - g), has_edges(has_edges){}
...
public:
        ...
        for (auto& pair : nodes) {
            ...
            while (grid[closest_y][closest_x]&& !found) {
                ...
                while (!points.empty()){
                    ...
                    int new_x = closest_x + random_point.first;
                    int new_y = closest_y + random_point.second;
    ...
    void compactNodes(std::vector<std::vector<std::string>>& grid, std::vector<Node*>& nodes) {
        for (auto& node : nodes) {
            int x = static_cast<int>(node->get_x());
            int y = static_cast<int>(node->get_y());
            ...
            while (true) {
                ...
                // 上方向に移動可能
                if (y > 0 && grid[y - 1][x] == "") {  // 空の場所を "" でチェック
                    grid[y][x] = "";  // 現在の位置を空にする
                    grid[y - 1][x] = node->get_id();  // 新しい位置にノードを移動
                    node->set_coordinates(x, y - 1);
                    y = y - 1;
                    moved = true;
                }
                // 下方向に移動可能
                ...
                // 左方向に移動可能
                    ...
                    grid[y][x] = "";
                    grid[y][x - 1] = node->get_id();
                    node->set_coordinates(x - 1, y);
                    x = x - 1;
                    moved = true;
                }
                // 右方向に移動可能
                else if (x < grid[0].size() - 1 && grid[y][x + 1] == "") {
                    grid[y][x] = "";
                    grid[y][x + 1] = node->get_id();
                    node->set_coordinates(x + 1, y);
                    x = x + 1;
                ...
                if (!moved) break;  // どの方向にも動けなかったらループ終了
            }
        }
    }
    ...
    void exportToDOT(const std::string& filename) {
        ...
        file << "ratio=1;\n" << "    node [shape=circle, width=1, height=1, fixedsize=true, fontsize=10];\n";  // 文字を小さく設定
