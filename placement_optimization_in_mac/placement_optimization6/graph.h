...
private:
    ...
    std::unordered_map<std::string, int> nodeWeights;

public:
    Graph() : grid(0, std::vector<bool>(0, false)) {}
    ...
    void calculateNodeWeights() {
        ...
        for (const auto& edge : edges) {
            nodeWeights[edge.get_node1()] += edge.get_weight();
            nodeWeights[edge.get_node2()] += edge.get_weight();
        }
    }

    std::vector<Node*> getSortedNodesByWeight() {
        std::vector<Node*> sortedNodes;
        for (auto& pair : nodes) {
            sortedNodes.push_back(pair.second);
        }
        std::sort(sortedNodes.begin(), sortedNodes.end(), [this](Node* a, Node* b) {
            return nodeWeights[a->get_id()] > nodeWeights[b->get_id()];
        });
        return sortedNodes;
    }

    void resizeGrid() {
        grid_size = std::ceil(std::sqrt(nodes.size()));
        grid.resize(grid_size, std::vector<bool>(grid_size, false));
    }
    // ノードを追加
    ...
    void layoutGraph() {
        ...
        while (total_movement > epsilon) { // 多くの反復で座標を調整
            ...
            for (const auto& edge : edges) {
                ...
                // 重みが大きいほどノード同士を近づける
                double force = pow(edge.get_weight(),2)*pow(distance, 1)*0.00001*epsilon;
                ...
                if (std::sqrt(pow(dx-std::abs(dp_x),2) + pow(dy-std::abs(dp_y), 2))<min_distance) {
                    ...
                    if (u_magic){
                        v->set_coordinates(vx - 2*dp_x, vy - 2*dp_y); 
                    } else if (v_magic){
                        u->set_coordinates(ux - 2*dp_x, uy - 2*dp_y); 
                    }else{
                        u->set_coordinates(ux + dp_x, uy + dp_y);
                        v->set_coordinates(vx - dp_x, vy - dp_y); 
                    }
                }else{
                    // ノードの座標を更新
                    if (u_magic){
    ...
    void snapToGrid() {
        ...
        std::vector<Node*> sortedNodes = getSortedNodesByWeight();
    ...
    void exportToDOT(const std::string& filename) {
        ...
        file << "ratio=1;\n" << "    node [shape=circle, width=2, height=2, fixedsize=true, fontsize=7];\n";  // 文字を小さく設定
